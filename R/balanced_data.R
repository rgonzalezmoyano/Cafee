...
balance_data <- function (
  ...
  if (N < 150) {
    ...
    # create new inefficient observations
    idx_dmu<- sample(1:nrow(data), size = n / 2)
  ...
  if (prop_eff > prop_ineff) {
    ...
    # project inefficient data 
    proj_data_out <- as.data.frame(cbind(grid[, x], grid[, y] * bcc_scores_out[, 1]))
    proj_data_inp <- as.data.frame(cbind(grid[, x] * bcc_scores_inp[, 1], grid[, y]))
    
    names(proj_data_out) <- names(grid[, c(x, y)])
    names(proj_data_inp) <- names(grid[, c(x, y)])
    ...
    # index to improve by orientation
    idx_eff <- sample(idx_eff, size = new_dmus)
    
    # output projection
    idx_eff_out <- idx_eff[idx_eff < (nrow(grid) - nrow(data))]
    
    # input projection
    idx_eff_inp <- setdiff(idx_eff, idx_eff_out)
